...
parameter = Dict(
	...
	"initial number of infected" => 10000
);
...
begin
	...
	change_parameter = Dict(
		"rate of infection" => parameter["rate of infection"],
		"measure on" => false
		)
end;

# ╔═╡ df5c124f-63fa-4755-a369-27ef7a90472a
#PARAMETER VON INFECTION RATE
function massnahmen(par,t,F,n)
	if t >10
		 m = ceil(Integer,(t-5)/Δt)
		if F[m,2] >0.01* par[1]["total population"]
			par[1]["rate of infection"] = par[4]["rate of infection"]/1.5
			par[4]["measure on"] = true
			
		end
		if par[4]["measure on"] &&  F[m,2] <0.005*  par[1]["total population"]
		par[1]["rate of infection"] = par[4]["rate of infection"]	
	par[4]["measure on"] = false
		end	
	end
end
...
begin
	...
	result = euler(
		...
		0.0,T,Δt,
		x₀,
		(parameter,parameter_subpopulation,mixing_parameter,change_parameter)
		)
end;
