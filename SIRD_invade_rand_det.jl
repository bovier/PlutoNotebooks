...
parameter = Dict(
	...
	"rate of invasion" => 0.1,
	"total population" => 1*10^8,
	"initial number of infected" => 0
);
...
begin
	...
	mixing_parameter = Dict(
		"rate of infection 1 -> 2" => 0,
		"rate of infection 2 -> 1" => 0
	)
end;
...
begin
	...
	SIR_step(t,x,par::Dict) = SIR_step(t,x,
				...
				par["rate of recovery"],par["rate of death of infected"],
	         	par["rate of invasion"],par["total population"]
			)
	
	SIR_step(t,H,n,par::Dict) = SIR_step(t,H[n,:],par)
end;

# ╔═╡ f7e71f11-e0ed-4a20-a704-8e7fb85e07d0
md"""
**Defines the infinitesimal change of deterministic system**
"""

# ╔═╡ 19092fd0-8d22-45be-9620-cc956822af73
begin
	function SIR_step_det(t,x,β,δ,γ,ρ,in,N)
		S, I, R, D = x[1], x[2], x[3], x[4]
		nI = β*I*S/N*Δt
		nIL = δ*R*Δt
		nR = γ*I*Δt
	...
	SIR_step_det(t,x,par::Dict) = SIR_step_det(t,x,
				par["rate of infection"],par["rate of immunity loss"],
				par["rate of recovery"],par["rate of death of infected"],
		        par["rate of invasion"],par["total population"]
			)
	
	SIR_step_det(t,H,n,par::Dict) = SIR_step_det(t,H[n,:],par)
end;

# ╔═╡ b58307a4-b36d-40e9-b406-2fdce8318284
...
begin
	function SIR_step_2D(t,H,n,par₁::Dict,par₂::Dict,par_mix::Dict)
		...
		x = vcat(SIR_step_det(t,x₁,par₁),SIR_step(t,x₂,par₂))
...
let
	p1 = plot(framestlye=:zerolines,title="Deterministic Simulation")
	...
	p2 = plot(framestlye=:zerolines,title="Random Population")
